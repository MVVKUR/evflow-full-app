...
def test_topup_status_poll_credits_when_xendit_reports_paid(monkeypatch):
    ...
    with TestClient(main.app) as c:
        before = c.get("/api/v1/wallet").json()["balance_idr"]
        created = c.post("/api/v1/wallet/topup", json={"amount_idr": 25000}).json()
        topup_id = created["topup_id"]

        # still pending at Xendit: poll reports pending, no credit
        polled = c.get(f"/api/v1/wallet/topups/{topup_id}").json()
        assert polled["status"] == "pending"
        assert c.get("/api/v1/wallet").json()["balance_idr"] == before
        ...
        assert c.get("/api/v1/wallet").json()["balance_idr"] == before + 25000
        ...
        assert c.get("/api/v1/wallet/topups/00000000-0000-0000-0000-000000000000").status_code == 404
