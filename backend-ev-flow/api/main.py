...
from .services import site_economics
...
_DEMAND_IS_YOURS = (
    "Demand is your assumption, not a forecast. This service does not predict how many "
    "sessions a new site would see, because there is almost no measured demand to learn "
    "from: nearly every charging session in the database is generated by a simulation "
    "script. The payback below is the arithmetic consequence of the figure you supplied, "
    "and it is only as good as that figure."
)


@app.post("/api/v1/planner/roi", response_model=SiteRoiResponse, tags=["planner"],
          summary="Payback projection for a site at one cell (AC Epic 5)",
          responses={404: {"description": "No such cell"},
                     422: {"description": "Inputs the arithmetic cannot support"}})
def planner_roi(body: SiteRoiInput,
                user: dict = Depends(require_planner)) -> SiteRoiResponse:
    """Project revenue and payback for a proposed site, and refuse when it cannot.

    This is a calculator rather than a forecast. It supplies the tariff the
    charging flow really bills at and the features of the cell as measured, does
    the arithmetic on the planner's own cost and demand assumptions, and labels
    every input with where it came from so the two are never confused.

    A site that does not break even gets a null payback rather than the negative
    number the naive division produces, and demand above what the connectors can
    physically serve is refused rather than quietly clamped down to the ceiling.
    """
    cell = planner_repo.get_cell(body.cell_id)
    if cell is None:
        raise HTTPException(404, f"cell '{body.cell_id}' not found")

    tariff = body.tariff_idr_per_kwh if body.tariff_idr_per_kwh is not None else pricing.base_rate_idr()
    admin_fee = body.admin_fee_idr if body.admin_fee_idr is not None else pricing.admin_fee_idr()

    try:
        if body.sessions_per_day is not None:
            sessions_per_day = body.sessions_per_day
            demand_basis = f"Sessions per day was supplied directly. {_DEMAND_IS_YOURS}"
        else:
            sessions_per_day = site_economics.sessions_from_utilisation(
                body.utilisation_target, body.connectors, body.power_kw,
                body.energy_per_session_kwh)
            demand_basis = (
                f"Sessions per day was derived from a utilisation target of "
                f"{body.utilisation_target:.0%} against the hardware specified. {_DEMAND_IS_YOURS}")

        projection = site_economics.project(site_economics.SiteEconomicsInput(
            connectors=body.connectors, power_kw=body.power_kw,
            sessions_per_day=sessions_per_day,
            energy_per_session_kwh=body.energy_per_session_kwh,
            capex_per_connector_idr=body.capex_per_connector_idr,
            opex_monthly_idr=body.opex_monthly_idr,
            tariff_idr_per_kwh=tariff, admin_fee_idr=admin_fee,
            energy_cost_idr_per_kwh=body.energy_cost_idr_per_kwh,
            horizon_years=body.horizon_years))
    except ValueError as exc:
        raise HTTPException(422, str(exc)) from exc

    # model_fields_set holds only what the caller actually sent, which is what
    # separates a number the planner chose from one this endpoint chose for them.
    sent = body.model_fields_set
    sources = {name: ("planner" if name in sent else "default")
               for name in ("connectors", "power_kw", "energy_per_session_kwh",
                            "horizon_years", "sessions_per_day", "utilisation_target",
                            "energy_cost_idr_per_kwh")
               if getattr(body, name, None) is not None}
    sources["capex_per_connector_idr"] = "planner"
    sources["opex_monthly_idr"] = "planner"
    sources["tariff_idr_per_kwh"] = ("planner" if body.tariff_idr_per_kwh is not None
                                     else "charging tariff configuration")
    sources["admin_fee_idr"] = ("planner" if body.admin_fee_idr is not None
                                else "charging tariff configuration")

    if body.energy_cost_idr_per_kwh == 0:
        cost_basis = (
            "Energy purchase cost is set to zero, so this projection assumes your opex figure "
            "already contains the electricity bill. If it does not, revenue here is the driver's "
            "price rather than what you keep, and the payback is too short.")
    else:
        # Group the digits only. Running the separator swap over the whole
        # sentence turns every comma in the prose into a full stop.
        bought = f"{body.energy_cost_idr_per_kwh:,}".replace(",", ".")
        sold = f"{tariff:,}".replace(",", ".")
        cost_basis = (
            f"Energy is bought at Rp {bought}/kWh and resold at Rp {sold}/kWh, so the margin "
            f"on energy is the spread between them.")

    return SiteRoiResponse(
        cell_id=cell["cell_id"], kota=cell["kota"], cost_basis=cost_basis,
        score=float(cell["score"]) if cell["score"] is not None else None,
        population=cell["population"], poi_total=cell["poi_total"],
        station_count=cell["station_count"],
        nearest_station_m=float(cell["nearest_station_m"]) if cell["nearest_station_m"] is not None else None,
        stations_2km=cell["stations_2km"],
        inputs=projection.pop("assumptions"), input_sources=sources,
        demand_basis=demand_basis, **projection,
    )
