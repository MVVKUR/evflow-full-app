...
export function PlanRouteScreen({ topInset = 0, bottomOffset = 0, onNavigationModeChange }: Props) {
  ...
  return <View style={styles.shell}>
    ...
    <DestinationSearchModal visible={Boolean(searchMode)} mode={searchMode ?? 'destination'} onClose={() => setSearchMode(null)} onSelect={selectLocation} onNetworkError={() => setConnectionError(true)} onConnectionRestored={() => setConnectionError(false)} originLat={origin?.latitude} originLon={origin?.longitude} />
